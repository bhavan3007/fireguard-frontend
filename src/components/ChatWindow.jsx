...
export default function ChatWindow() {
  ...
  const loadChatSession = async (id) => {
    ...
    const res = await fetch(`https://fireguard-backend.onrender.com/session/${id}`);
  ...
  const createSession = async () => {
    ...
    const res = await fetch("https://fireguard-backend.onrender.com/session", {
      method: "POST",
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      if (currentSession && messages.length === 0 && user) {
        await fetch("https://fireguard-backend.onrender.com/session/title", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
